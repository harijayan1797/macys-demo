...
data "template_file" "phpconfig" {
  ...
  template = templatefile("${path.module}/configurations/conf.wp-conf.php", { db_host = "${google_sql_database_instance.sql.public_ip_address}", db_name = "${google_sql_database.database.name}", db_user = "${google_sql_user.users.name}", db_pass = "${google_sql_user.users.password}" })
...
data "template_file" "filebeat" {
  ...
  template = templatefile("${path.module}/configurations/filebeat.yml", { ip = "${google_compute_instance.elk.network_interface.0.network_ip}" })
